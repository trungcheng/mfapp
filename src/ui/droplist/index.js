...
class DropList extends Component {
	...
	buildData(data = []) {
		...
		list = list.filter(l => ignores.findIndex(v => v == l.value) == -1);
	...
	handleSearch(keyword) {
		...
		let searchData = clone(rawData);

		if (keyword) {
			const inputVal = removeUnicode(keyword.toLowerCase());

			searchData = rawData.filter(item => {
				const inputItem = removeUnicode(item.label.toLowerCase());
	...
	renderData(list, level = 1) {
		if (isArray(list) && !isEmpty(list)) {
			...
			if (isArray(value)) {
				list.forEach(item => {
					if (value.findIndex(v => v ==item.value) != -1) {
			...
			list.forEach(item => {
				...
				if (disableValues === item.value) {
					...
					&& disableValues.findIndex(v => v == item.value) != -1) {
					classesItem += ' disabled';
				}
				...
				template.push(
					<div styleName={`droplist-item${classesItem}`}
						key={item.value}>
						<div 
							styleName="item-inner" style={styleItem}
							onClick={() => this.handleSelect(item.value)}
							styleName={`${values.findIndex(v => v == item.value) != -1 ? 'selected' : ''}${classesActive}`}
						>
							{
			...
			return (
				<div ref={level === 1 ? (c) => this.refDropList = c : ''}>
					{template}
				</div>
			);
		} else {
